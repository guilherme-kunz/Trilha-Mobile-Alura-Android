...
class LoginFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        configuraBotaoLogin()
        configuraBotaoCadrasto()
    }

    private fun configuraBotaoCadrasto() {
    ...
    private fun configuraBotaoLogin() {
        login_botao_logar.setOnClickListener {
            ...
            val email = login_email.editText?.toString()
            val senha = login_senha.editText?.toString()
        ...
        viewModel.autentica(Usuario(email, senha)).observe(viewLifecycleOwner, Observer {
            it?.let { recurso ->
                if (recurso.dado) {
                    ...
                    val mensagemErro = recurso.erro ?: "Erro durante a autentificação"
    ...
    private fun validaCampos(email: String?, senha: String?): Boolean {
        ...
        if (senha.isBlank()) {
            login_senha.error = "Senha obrigatória"
            valido = false
        }
        return valido
    }
